...
interface ClaimPopItemProps {
  contractName: ContractName;
  rewardAmount: number;
  claimRewards: () => void;
}

export function ClaimPopItem({
  contractName,
  rewardAmount,
  claimRewards,
}: ClaimPopItemProps): JSX.Element {
  ...
  return (
    <li
      className="gap-y-1 flex flex-row bg-white rounded-lg m-2 border"
      key={contractName}
    >
      ...
      <div className="w-1/4 whitespace-nowrap flex flex-wrap content-center justify-end">
        <button
          ...
          className={
            rewardAmount > 0
              ? 'mx-6 px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'
              : 'disabled:opacity-50 mx-6 px-4 py-2 border border-transparent text-sm font-medium rounded-md  text-gray-400 bg-indigo-200'
          }
          onClick={() =>
            rewardAmount > 0
              ? triggerClaimReward({
