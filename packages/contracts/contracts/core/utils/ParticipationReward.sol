...
contract ParticipationReward is ReentrancyGuard {
  ...
  function isClaimable(bytes32 _vaultId, address _beneficiary)
    public
    view
   ...
   */
  function hasClaim(bytes32 _vaultId, address _beneficiary)
    public
    view
    vaultExists(_vaultId)
    returns (bool)
   ...
   * @param _vaultId Bytes32
   */
  function getVaultStatus(bytes32 _vaultId)
    external
    view
    returns (VaultStatus)
  {
   ...
   */
  function getUserVaults(address _account)
    external
    view
    ...
    vaultExists(_vaultId)
  {
    ...
    require(
      vaults[_vaultId].endTime <= block.timestamp,
   ...
   */
  function addControllerContract(bytes32 _contractName, address _contract)
    external
  {
    IACLRegistry(contractRegistry.getContract(keccak256("ACLRegistry")))
      .requireRole(keccak256("DAO"), msg.sender);
