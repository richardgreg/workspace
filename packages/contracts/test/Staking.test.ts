...
describe("Staking", function () {
  beforeEach(async function () {
    ...
    const StakingDefendedHelper = await ethers.getContractFactory(
      "StakingDefendedHelper"
    );
  ...
  describe("stake", function () {
    it("should reject zero amount", async function () {
      await expect(staking.stake(0, 7 * DAY)).to.be.revertedWith(
        "amount must be greater than 0"
      );
    });

    it("should lock for at least a week", async function () {
      await expect(staking.stake(1, 7 * DAY - 1)).to.be.revertedWith(
    ...
    it("should error on insufficient balance", async function () {
      await expect(
        staking.stake(parseEther("10000000000"), 7 * DAY)
    ...
    it("should lock funds successfully", async function () {
      ...
      expect(await staking.getVoiceCredits(owner.address)).to.equal(
        parseEther("47.945205479452054794")
      );
    });
  ...
  describe("updatePeriodFinish", function () {
    ...
    it("should increase staking period", async function () {
      ...
      await staking
        .connect(owner)
        .updatePeriodFinish(periodFinish.add(7 * DAY));
      await expect(await staking.periodFinish()).to.equal(
        periodFinish.add(7 * DAY)
      );
    });
